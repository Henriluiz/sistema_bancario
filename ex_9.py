...
class Autenticator:
    ...
    def auth_senha(self, conta=False):
...
class ContaBancaria(Autenticator):
    ...
    FILE = Path(__file__).parent / 'ex_9_treinamento.json'
    taxa_juros = 1.09
    ...
    def __init__(self):
        ...
        if not sair: # Criando o cadastro, se quiser sair e ele pulará essa parte
            ...
            antigo = ContaBancaria._numero_conta(self,num_conta)
    ...
    def _numero_conta(self,valor):
        ...
        if ContaBancaria.buscar_por_numero(numero_conta=valor):
            cont = str(input(f"Essa conta já existe, pretende continuar [S/N]? ")).upper().startswith("SIM")
            if cont:
                while True:
    ...
    def compra_com_credito(self, item="", valor_compra=0.0):
        ...
        self._registro[0][f'compra em {item}'] = float(valor_compra)
        self._registro[1].append(float(-valor_compra))
    ...
    def compra_com_debito(self, item="", valor_compra=0.0):
        ...
        self._registro[0][f'compra em {item} (Debito)'] = valor_compra
        self._registro[1].append(-valor_compra)
...
class Transacao(ContaBancaria, Autenticator):
    def depositar(self, valor_depositar=0.0):
        ...
        ContaBancaria.contas[self.numero_conta]["_saldo"] += valor_depositar
...
print(a.desbloquear_conta())
print(a.solicitar_cartao_credito())
print(a.compra_com_credito("Casa", 300000))


# TODO: Menu de Lembrete! Organizar e fazer outras classes. (Transação / Autenticator / DataStorage)
