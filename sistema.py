    ...
    def auth_senha(self, senha="", conta=False):
        ...
        if not conta:  # Vericação da existência da instância
            try:
                ...
                print(
                    '\033[1;31mSua Instância não tem os dados necessário. Use ContaBancaria primeiro e cadastre todas as informações necessária lá!\033[m')
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 12, 'Fatura de Compras',
    ...
    def fatura(self, numero_conta, registro):
        ...
        self.cell(
            0, 10, f"Fatura da conta: {numero_conta}", ln=True, align='C')
        self.ln(5)
        ...
        self.cell(largura_valor, 10, 'Valores (R$)',
        ...
        for item, valor in registro.items():
            ...
            self.cell(largura_valor, 10,
                      f"{valor:.2f}", border=1, align='R', fill=fill)
            self.ln()
...
class ContaBancaria(Autenticator):
    ...
    def __init__(self, num_conta="", ):
        ...
        while True:
            try:
                # num_conta = int(input('Digite o número da conta [Conta nova/Antiga]: '))
                num_conta = 15245
                ...
                print(
                    "\033[1;31m<<< Digite apenas números nessa entrada >>>\033[m")
            else:
                break
        ...
        if not sair:  # Criando o cadastro, se quiser sair e ele pulará essa parte
            ...
            ContaBancaria.contas[num_conta] = {
                chave: valor for chave, valor in self.__dict__.items() if chave != "_numero_conta"}
        ...
        if ContaBancaria.buscar_por_numero(numero_conta=valor):
            if senha:
                if ContaBancaria.auth_senha(self, senha, conta=valor):
                    ...
                    print(
                        f'\033[mVocê inseriu uma conta já existente, tente novamente com o número correto\033[m')
                return "Conta nova"
            else:
    ...
    def copiar_conta(self, num):
        try:
            ...
            print("2º self._registro>>>>>>",
                  ContaBancaria.contas[num]["_registro"])
    ...
    def consultar_extrato(self):
        ...
        print("3º lista_self", lista_self)
    ...
    def abrir_chave_pix(self, metodo=1, nova_chave=""):
        ...
        if not nova_chave:
            while True:
                try:
                    nova_chave = str(
                        input('Digite sua nova chave pix [Telefone/Email/CPF]: '))
                    metodo = int(
                        input('Digite o formato da chave[1 - CPF, 2 - Telefone, 3 - Email]: '))
                except Exception:
        ...
        if not item or not valor_compra:
            while True:
                try:
                    ...
                    print(
                        '\033[1;31mDigite apenas texto na primeira entrada e float na segunda!\033[m')
                else:
                    break
        else:
            try:
                valor_compra = float(valor_compra)
                # Se o item vira inteiro, significa que só tem números aqui
                item = int(item)
                ...
                print(
                    '\033[1;31mDigite apenas texto no primeiro argumento e float no segundo argumento\033[m')
            ...
            while True:
                try:
                    valor_depositar = float(
                        input('Digite o valor para depositar: '))
                except Exception:
    ...
    def transferir(self, conta_destino="", valor_de_transferencia=0.0):
        ...
        if not conta_destino and valor_de_transferencia:
            ...
            while True:
                try:
                    conta_destino = str(
                        input('Digite o número da conta destino: '))
                    conversao = int(conta_destino)
                    valor_de_transferencia = float(
                        input('Digite o valor da transfêrencia: '))
                    print(
                        f"Confirme seus dados.\nConta Destino: {conta_destino}\nValor da Transferência: {valor_de_transferencia}")
                    confirmar = str(
                        input('Digite "S" para Confirmar ["N" para refazer]: ')).upper().startswith("S")
                except Exception:
                    print(
                        'Digite conta destino apenas com números inteiros, e valor em float')
                else:
                    if confirmar:
                        break

        conta_encontrada, num_conta, pix = ContaBancaria._validar_conta(
            conta_destino)
        ...
        ContaBancaria.contas[num_conta]["registro"][1].append(
            valor_de_transferencia)  # Registrando o ganho do usuário

        # Arredondamento a taxa
        taxa = round(valor_de_transferencia * ContaBancaria.taxa_juros, 2)
        ...
        try:
            ...
            for item in dados:
                for itens in dados[item]:
                    if itens == "_senha":
                        dados[item]["_senha"] = Autenticator.descriptografar(
                            dados[item]["_senha"])
    ...
    def salvar(cls):
        ...
        for item in cls.contas:
            for itens in cls.contas[item]:
                if itens == "_senha":
                    cls.contas[item]["_senha"] = cls.criptografar(
                        cls.contas[item]["_senha"])
